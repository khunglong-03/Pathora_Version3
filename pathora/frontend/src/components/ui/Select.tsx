    ...
    >
      ...
      <div className={`relative ${horizontal ? "flex-1" : ""}`}>
        <select
          ...
          {...rest}
          name={name}
          className={`form-control appearance-none py-2 border border-input bg-background text-foreground ${
            error ? "has-error" : ""
          } ${className}`}
          disabled={disabled}
          id={id}
        ...
        <div className="pointer-events-none absolute top-1/2 flex -translate-y-1/2 space-x-1 text-xl ltr:right-3.5 rtl:left-3.5 rtl:space-x-reverse">
          <span className="relative -right-2 inline-block text-foreground">
        ...
        <div
          ...
          className={`mt-2 ${
            msgTooltip
              ...
              : "text-destructive block text-sm"
          }`}
        >
          {error.message}
        </div>
      ...
      {validate && (
        <div
          className={`mt-2 ${
            msgTooltip
              ? "bg-success inline-block rounded-sm px-2 py-1 text-[10px] text-success-foreground"
              : "text-success block text-sm"
          }`}
        >
          {validate}
        </div>
