...
export const orderService = {
  getOrders: <T = unknown[]>(): Promise<ServiceResponse<T>> => {
    return executeApiRequest<T>(() => api.get(API_ENDPOINTS.ORDER.GET_LIST));
  },

  getAllOrders: <T = unknown[]>(): Promise<ServiceResponse<T>> => {
    return executeApiRequest<T>(() => api.get(API_ENDPOINTS.ORDER.GET_ALL));
  },

  getOrderDetail: <T = unknown>(id: string): Promise<ServiceResponse<T>> => {
