...
export function BookingSearchFilter({
  ...
  return (
    <motion.div 
      ...
      className={cn("mb-10 rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-8")}
    >
      <div className={cn("v-stack justify-between gap-8 lg:h-stack lg:items-center")}>
        ...
        <div className={cn("group relative max-w-xl flex-1")}>
          <MagnifyingGlass
            weight="bold"
            className={cn("absolute left-4 top-1/2 z-10 size-5 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-slate-900")}
          />
          <TextInput
            type="text"
            ...
            className={cn("w-full !rounded-lg !border-slate-200 !bg-slate-50 !py-3.5 !pl-12 !pr-4 text-base font-medium transition-all placeholder:!text-slate-400 focus:!border-slate-900 focus:!bg-white focus:!ring-slate-900/10")}
          />
        </div>
                ...
                >
                  {isActive && (
                    <motion.div
                      ...
                      className={cn("absolute inset-0 rounded-full bg-[#111111]")}
                      initial={false}
