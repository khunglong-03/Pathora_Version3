...
import { Users, User, WarningCircle, UserCirclePlus, Spinner, IdentificationCard, CheckCircle, Info } from "@phosphor-icons/react";
import { BookingDetail } from "./BookingDetailData";
import { BookingParticipantSummary } from "@/types/booking";
...
export function GuestDetailsCard({ 
  ...
  const [participants, setParticipants] = useState<BookingParticipantSummary[]>([]);
  ...
  const countParts = [
    booking.adults > 0 && t("landing.bookingDetail.passengerCount.adults", { count: booking.adults }),
    booking.children > 0 && t("landing.bookingDetail.passengerCount.children", { count: booking.children }),
    (booking.infants ?? 0) > 0 && t("landing.bookingDetail.passengerCount.infants", { count: booking.infants }),
  ].filter(Boolean).join(" · ");

  const sortedParticipants = [...participants].sort((a, b) => {
    const rank = (s: string) => s === "Rejected" ? 0 : s === "NotReviewed" ? 1 : 2;
    const cmp = rank(a.infoReviewStatus) - rank(b.infoReviewStatus);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-10">
        ...
        <h2 className="text-2xl font-bold tracking-tight text-slate-900 flex flex-wrap items-center gap-x-2">
          <span>{t("landing.bookingDetail.guestDetails")}</span>
          {countParts && (
            <>
              <span className="text-slate-300 font-normal">·</span>
              <span className="text-sm text-slate-500 font-medium">{countParts}</span>
            </>
          )}
        </h2>
      </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 space-y-1">
                    {participants.filter(p => p.infoReviewStatus === "Rejected").map((p, idx) => (
                      <div key={p.participantId || idx} className="text-xs text-red-700 font-medium">
