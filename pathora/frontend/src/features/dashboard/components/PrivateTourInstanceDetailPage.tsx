...
function ApprovalSummaryBar({
  ...
  return (
    <div className="bg-white rounded-[1.5rem] border border-slate-200/60 p-5 space-y-3">
      <div className="h-stack items-center gap-2 text-sm font-semibold text-slate-700">
        <Icon icon={icon} className="size-4 text-slate-400" />
        {label}
        <span className="text-slate-400 font-normal">({summary.total})</span>
      </div>
...
export default function PrivateTourInstanceDetailPage() {
  ...
  const isPostApproval = useMemo(() => {
    if (!data?.status) return false;
    return ["pendingvisa", "confirmed", "available", "inprogress", "operating", "completed"].includes(data.status.toLowerCase());
  }, [data]);

  /* ── Loading ──────────────────────────────────────────────── */
  if (dataState === "loading") {
    return (
      <main className="py-8 md:py-10 px-4 md:px-6 lg:px-8 max-w-7xl mx-auto space-y-5">
        <SkeletonCard />
  ...
  if (dataState === "error" || !data) {
    return (
      <main className="py-8 md:py-10 px-4 md:px-6 lg:px-8">
        <div className="mx-auto max-w-xl rounded-[1.5rem] border border-slate-200 bg-white p-10 text-center shadow-md">
          <Icon icon="heroicons:exclamation-circle" className="mx-auto mb-3 size-12 text-red-400" />
          <p className="text-base font-semibold text-slate-900">Không thể tải thông tin tour</p>
          {errorMessage && <p className="mt-2 text-sm text-slate-500">{errorMessage}</p>}
          <div className="mt-6 h-stack items-center justify-center gap-3">
            <button
  ...
  /* ── Ready ────────────────────────────────────────────────── */
  return (
    <main className="py-8 px-4 md:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      {/* ── Breadcrumb ───────────────────────────────────────── */}
      <motion.nav
        variants={itemVariants}
        initial="hidden"
        animate="show"
        className="h-stack items-center gap-2 text-sm text-slate-500"
        aria-label="Breadcrumb"
      >
        <Link
          href={backHref}
          className="h-stack items-center gap-1.5 hover:text-slate-900 transition-colors"
        >
      ...
      <motion.div
        ...
        initial="hidden"
        animate="show"
        className="v-stack md:h-stack md:items-start gap-5"
      >
        {/* Thumbnail */}
        <div className="w-full md:w-40 h-32 md:h-28 rounded-[1.5rem] overflow-hidden bg-slate-100 border border-slate-200/60 shrink-0">
          {data.thumbnail?.publicURL ? (
            ...
            <div className="w-full h-full h-stack items-center justify-center text-slate-300">
        ...
        {/* Title & meta */}
        <div className="spacer min-w-0 space-y-2">
          <div className="flex items-start gap-3">
            <h1 className="text-3xl md:text-4xl font-bold tracking-tighter text-slate-900 leading-tight spacer">
              {data.title || data.tourName}
            </h1>
            <TourStatusBadge status={data.status} />
          </div>
          <p className="text-sm text-slate-400 font-mono">{data.tourInstanceCode}</p>
          <div className="h-stack flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-600 mt-1">
      ...
      {data.customizationNotes && (
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="bg-amber-50 border border-amber-100 rounded-[1.5rem] p-5 space-y-2"
        >
          <div className="h-stack items-center gap-2 text-xs font-bold text-amber-700 uppercase tracking-wider">
        ...
        >
          <div className="h-stack items-center justify-between">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-amber-500 text-white text-xs font-semibold hover:bg-amber-600 active:scale-[0.97] transition-all shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]"
            >
            ...
            {addingDay && (
              <motion.div key="form" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="bg-white border border-amber-200 rounded-[1.5rem] p-5 space-y-3 overflow-hidden">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setAddingDay(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 transition-colors">Huỷ</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {data.days.map((day, dayIdx) => (
            <motion.div
              key={day.id ?? dayIdx}
              variants={itemVariants}
              className="bg-white border border-slate-200/60 rounded-[1.5rem] overflow-hidden"
            >
              {/* Day header */}
              <div className="px-5 py-4 border-b border-slate-100 h-stack items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-700 h-stack items-center justify-center text-sm font-bold shrink-0">
                  {dayIdx + 1}
                </div>
                <div className="spacer min-w-0">
                  <h3 className="text-base font-semibold text-slate-900 truncate">{day.title}</h3>
                  {day.actualDate && <p className="text-xs text-slate-500">{formatDate(day.actualDate)}</p>}
                </div>
                <button onClick={() => startEditDay(day)} className="p-1.5 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-colors" title="Sửa ngày">
                ...
                {editingDayId === day.id && (
                  <motion.div key="form" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="px-5 py-4 border-b border-amber-100 bg-amber-50/30 space-y-3 overflow-hidden">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setEditingDayId(null)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Huỷ</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
                  ...
                  {day.activities.map((act, aIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div className="spacer min-w-0 space-y-1.5">
                          <div className="h-stack items-center gap-2 flex-wrap">
                          ...
                          {/* Supplier info */}
                          {supplierName && (
                            <p className="text-xs text-slate-500 h-stack items-center gap-1">
                          ...
                          {isTransport && (
                            <div className="h-stack flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                        ...
                        {/* Edit/Delete buttons */}
                        <div className="h-stack items-center gap-1 shrink-0 opacity-0 group-hover/act:opacity-100 transition-opacity">
                          <button onClick={(e) => { e.stopPropagation(); startEditActivity(act); }} className="p-1 rounded-md text-slate-400 hover:text-amber-600 hover:bg-amber-50" title="Sửa">
                            <Icon icon="heroicons:pencil-square" className="size-3.5" />
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); deleteActivity(day.id, act.id); }} className="p-1 rounded-md text-slate-400 hover:text-red-600 hover:bg-red-50" title="Xoá">
                        ...
                        {editingActivityId === act.id && (
                          <motion.div key="form" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="px-5 py-3 border-t border-amber-100 bg-amber-50/20 space-y-3 overflow-hidden">
                            ...
                            <div className="flex gap-2">
                              ...
                              <button onClick={() => setEditingActivityId(null)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Huỷ</button>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                ...
                {addingActivityForDayId === day.id && (
                  <motion.div key="form" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="px-5 py-4 border-t border-amber-100 bg-amber-50/20 space-y-3 overflow-hidden">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setAddingActivityForDayId(null)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">Huỷ</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      <motion.div
        ...
        initial="hidden"
        animate="show"
        className="v-stack sm:flex-row gap-3 pt-4"
      >
        {!isPostApproval && (
          <Link
            href={editHref}
            className="spacer h-stack items-center justify-center gap-2 py-3.5 rounded-[1.5rem] bg-amber-500 text-white text-sm font-semibold transition-all hover:bg-amber-600 hover:-translate-y-0.5 active:scale-[0.98] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]"
          >
        ...
        {isPostApproval && (
          <>
            <Link
              href={`/tour-operator/tour-instances/private/${id}/book-transport`}
              className="spacer h-stack items-center justify-center gap-2 py-3.5 rounded-[1.5rem] bg-blue-600 text-white text-sm font-semibold transition-all hover:bg-blue-700 hover:-translate-y-0.5 active:scale-[0.98] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]"
            >
              <Icon icon="heroicons:truck" className="size-5" />
              Nhập vé máy bay / xe
            </Link>
            <Link
              href={`/tour-operator/tour-instances/private/${id}/book-accommodation`}
              className="spacer h-stack items-center justify-center gap-2 py-3.5 rounded-[1.5rem] bg-purple-600 text-white text-sm font-semibold transition-all hover:bg-purple-700 hover:-translate-y-0.5 active:scale-[0.98] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]"
            >
              <Icon icon="heroicons:building-office-2" className="size-5" />
              Đặt phòng khách sạn
            </Link>
            <Link
              href={`/tour-operator/tour-instances/private/${id}/assign-services`}
              className="spacer h-stack items-center justify-center gap-2 py-3.5 rounded-[1.5rem] bg-emerald-600 text-white text-sm font-semibold transition-all hover:bg-emerald-700 hover:-translate-y-0.5 active:scale-[0.98] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)]"
            >
              <Icon icon="heroicons:user-group" className="size-5" />
              Giao Tour Guide
            </Link>
          </>
        )}
        ...
        <Link
          href={backHref}
          className="spacer h-stack items-center justify-center py-3.5 rounded-[1.5rem] border border-slate-200/80 bg-white text-sm font-medium text-slate-600 transition-all hover:bg-slate-50 hover:text-slate-900 active:scale-[0.98]"
        >
          Quay lại danh sách
        </Link>
      </motion.div>
