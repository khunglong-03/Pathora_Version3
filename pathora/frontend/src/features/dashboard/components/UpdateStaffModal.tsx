...
import { XIcon, PencilSimpleLine } from "@phosphor-icons/react";
import type { UpdateStaffRequest } from "@/api/services/adminService";
...
export function UpdateStaffModal({ isOpen, onClose, onSubmit, staffData }: UpdateStaffModalProps) {
  ...
  useEffect(() => {
    if (staffData && isOpen) {
      ...
      setStaffType(staffData.role === "Tour Guide" ? 2 : 1);
      setPassword("");
      setErrors({});
      setApiError(null);
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#E5E7EB]">
          <div className="flex items-center gap-3">
            ...
            >
              <PencilSimpleLine size={18} weight="bold" style={{ color: currentRole.text }} />
            </div>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="************"
                className="w-full px-3 py-2.5 rounded-lg border border-[#E5E7EB] text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500/20 text-[#111827]"
              />
            </div>
          </div>
