  ...
  return (
    <section className={cn("py-24 md:py-32")}>
      <SectionContainer>
        ...
        <div className={cn("flex gap-5 overflow-x-auto snap-x snap-mandatory pb-4 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide")}>
          {tours.map((tour, idx) => (
            <ScrollReveal
              key={tour.id}
              delay={idx * 80}
              className={cn("snap-start shrink-0 w-[300px] md:w-[340px]")}
            >
              <Link href={`/tours/${tour.id}`} className={cn("block h-full")}>
                <div className={cn("group rounded-[1.25rem] bg-gray-100/50 dark:bg-white/5 border border-gray-200/40 dark:border-white/8 p-1 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:shadow-xl hover:shadow-black/5 hover:-translate-y-1 h-full")}>
                  <div className={cn("rounded-[calc(1.25rem-0.25rem)] bg-white dark:bg-[#1a1a2e] overflow-hidden h-full")}>
                    {/* Image */}
                    <div className={cn("relative h-44 overflow-hidden")}>
                      {tour.thumbnail ? (
                        ...
                        <div className={cn("absolute inset-0 bg-gradient-to-br from-teal-200/60 via-cyan-200/40 to-blue-200/30")} />
                      )}
                      <div className={cn("absolute inset-0 bg-gradient-to-t from-black/30 to-transparent")} />

                      {/* NEW badge */}
                      <div className={cn("absolute top-3 left-3 bg-emerald-500/90 backdrop-blur-sm rounded-full px-3 py-1 text-[10px] font-bold text-white uppercase tracking-wider")}>
                        New
                      </div>
                      {tour.isVisa && (
                        <div className={cn("absolute bottom-3 left-3 rounded-full bg-white/95 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#fa8b02] shadow-sm backdrop-blur-sm")}>
                    ...
                    <div className={cn("p-5")}>
                      ...
                      {tour.shortDescription && (
                        <p className={cn("mt-2 text-xs text-gray-500 dark:text-gray-400 line-clamp-2 leading-relaxed")}>
                          {tour.shortDescription}
                        </p>
                      )}
                      <div className={cn("mt-3 flex items-center gap-1.5 text-xs text-gray-400")}>
                        <svg className={cn("w-3.5 h-3.5")} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
