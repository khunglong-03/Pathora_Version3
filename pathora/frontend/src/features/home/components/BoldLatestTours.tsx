...
export const BoldLatestTours = () => {
  ...
  return (
    <section className="py-24 md:py-32 bg-white overflow-hidden border-t border-stone-100">
      <div className="max-w-[90rem] mx-auto px-6 md:px-12">
        ...
        >
          <div className="max-w-2xl">
            <span suppressHydrationWarning className="inline-block px-4 py-1.5 rounded-full bg-stone-100 text-[11px] font-bold text-stone-600 uppercase tracking-[0.2em] mb-4 border border-stone-200/50 shadow-sm">
          ...
          <Link
            href="/tours"
            className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-stone-900 border border-stone-900 rounded-xl text-white hover:bg-stone-800 transition-all font-bold text-sm shadow-sm hover:shadow-md active:scale-95 group shrink-0"
          >
        ...
        {error ? (
          ...
          >
            {Array.from({ length: 6 }).map((_, idx) => (
              <div key={idx} className="snap-center shrink-0 w-[280px] md:w-[320px] animate-pulse">
                <div className="h-[200px] md:h-[240px] rounded-[1.5rem] mb-4 bg-stone-100" />
          ...
          >
            {latestTours!.map((tour, idx) => (
              <Link
                key={tour.id}
                href={`/tours/${tour.id}`}
                className="snap-center shrink-0 w-[280px] md:w-[320px] group transition-all duration-1000"
                style={{
              ...
              >
                <div className="relative h-[200px] md:h-[240px] rounded-[1.5rem] overflow-hidden mb-5 bg-stone-100 shadow-sm border border-stone-200/50">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-stone-900/60 via-stone-900/10 to-transparent transition-opacity duration-500 group-hover:opacity-80" />
                  
                  <div className="absolute bottom-4 left-4 px-3 py-1.5 rounded-full bg-white/95 backdrop-blur-md text-stone-900 text-[10px] font-bold uppercase tracking-wider shadow-sm">
                    {formatBadgeDate(tour.createdAt)}
                  </div>
                  
                  {tour.isVisa && (
                    <div className="absolute top-4 left-4 rounded-full bg-orange-50/95 backdrop-blur-md px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-orange-700 shadow-sm border border-orange-200/50">
                      {t("landing.tourDetail.visaRequired", "Visa Required")}
                    </div>
                  )}
                </div>
                
                <h3 className="text-stone-900 font-bold text-[17px] mb-2 leading-snug group-hover:text-stone-600 transition-colors line-clamp-2">
                  {tour.tourName}
                </h3>
                
                <p className="text-stone-500 text-[13px] font-bold uppercase tracking-wider flex items-center gap-1.5">
