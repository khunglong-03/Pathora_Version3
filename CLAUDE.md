...
> **Cache Invalidation:** Khi viết command implement `ICacheInvalidator`, hãy chú ý thiết lập các cache keys cần xóa. `CacheInvalidator` hoạt động theo cơ chế exact-match thay vì prefix-match. Do đó, các command chỉnh sửa dữ liệu liên quan (như `UpdateParticipantCommand`, `UpdateCustomerPassportCommand`) phải invalidate chính xác cache key chi tiết (ví dụ `$"{CacheKey.Booking}:participants:{BookingId}"`) thay vì chỉ dùng prefix chung. Một follow-up change `fix-participant-cache-stale` được lên kế hoạch để chuẩn hóa các command legacy bị ảnh hưởng.
