...
public sealed record ApproveOperatorResponseCommand(
...
public sealed class ApproveOperatorResponseCommandHandler(
    ITourItineraryFeedbackRepository feedbackRepository,
    ITourInstanceRepository tourInstanceRepository,
    IOwnershipValidator ownershipValidator,
    Domain.UnitOfWork.IUnitOfWork unitOfWork,
    Microsoft.Extensions.Logging.ILogger<ApproveOperatorResponseCommandHandler> logger,
    ITourInstanceNotificationBroadcaster? notifications = null)
    : IRequestHandler<ApproveOperatorResponseCommand, ErrorOr<Success>>
{
    public async Task<ErrorOr<Success>> Handle(
        ApproveOperatorResponseCommand request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            feedback.Approve(userId);
            await feedbackRepository.UpdateAsync(feedback, cancellationToken);
            await unitOfWork.SaveChangeAsync(cancellationToken);

            if (notifications != null && feedback.Booking?.UserId != null)
            {
                try
                {
                    await notifications.NotifyItineraryFeedbackEventAsync(
                        request.TourInstanceId, feedback.Id, "Approved", $"user:{feedback.Booking.UserId}", null, cancellationToken);
                }
                catch (Exception ex)
                {
                    logger.LogWarning(ex, "Failed to broadcast Approved event for feedback {FeedbackId}", feedback.Id);
                }
            }
