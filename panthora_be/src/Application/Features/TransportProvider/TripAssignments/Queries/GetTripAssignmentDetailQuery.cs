...
public sealed class GetTripAssignmentDetailQueryHandler(
        ITourDayActivityRouteTransportRepository repository)
    : IQueryHandler<GetTripAssignmentDetailQuery, ErrorOr<TripAssignmentDetailDto>>
{
    public async Task<ErrorOr<TripAssignmentDetailDto>> Handle(
        GetTripAssignmentDetailQuery request,
