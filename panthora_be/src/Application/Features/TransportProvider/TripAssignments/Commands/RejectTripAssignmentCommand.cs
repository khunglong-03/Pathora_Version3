...
public sealed class RejectTripAssignmentCommandHandler(
        ITourDayActivityRouteTransportRepository repository,
        IUnitOfWork unitOfWork)
    : ICommandHandler<RejectTripAssignmentCommand, ErrorOr<TripAssignmentDetailDto>>
{
    public async Task<ErrorOr<TripAssignmentDetailDto>> Handle(
        RejectTripAssignmentCommand request,
        CancellationToken cancellationToken)
    {
        ...
        entity.Reject(request.Request.Reason, request.CurrentUserId.ToString());
