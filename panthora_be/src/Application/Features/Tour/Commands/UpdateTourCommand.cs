...
public sealed record UpdateTourCommand(
    ...
    [property: JsonPropertyName("ifUnmodifiedSince")] DateTimeOffset? IfUnmodifiedSince = null,
    bool IsManager = false) : ICommand<ErrorOr<Success>>, ICacheInvalidator
{
    ...
    : ICommandHandler<UpdateTourCommand, ErrorOr<Success>>
{
    ...
    {
        return await tourService.Update(request, request.IsManager);
    }
}
