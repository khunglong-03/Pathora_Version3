...
public interface IVehicleRepository : IRepository<VehicleEntity>
{
    ...
    Task<List<VehicleEntity>> FindAllByOwnerIdPaginatedAsync(Guid ownerId, int pageNumber, int pageSize, bool? isActive, Domain.Enums.Continent? locationArea, CancellationToken cancellationToken = default);
    Task<int> CountAllByOwnerIdAsync(Guid ownerId, bool? isActive, Domain.Enums.Continent? locationArea, CancellationToken cancellationToken = default);
